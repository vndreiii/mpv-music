...
local cover_filenames = { "cover.png", "cover.jpg", "cover.jpeg",
    ...
    for key, value in pairs(options or {}) do
        ...
        mp.msg.info("Sending notification with summary: " .. summary)
        local res = utils.subprocess({
            args = {"/usr/bin/notify-send", summary, body, unpack(option_args)}
        })
        if res.status ~= 0 then
            mp.msg.error("Failed to send notification. Subprocess failed with status: " .. res.status)
            if res.stdout then mp.msg.error("notify-send stdout: " .. res.stdout) end
                if res.stderr then mp.msg.error("notify-send stderr: " .. res.stderr) end
                    return
                    end
                    mp.msg.info("Notification sent successfully.")
                    end
                            ...
                            for _, fname in ipairs(cover_filenames) do
                                ...
                                if file_exists(path) then
                                    ...
                                    if not file_exists(out_path) then
                                        ...
                                        mp.msg.info("notify_current_media called")
                                        ...
                                        if metadata then
                                            ...
                                            if album then origin = string.format("%s — %s", origin, album) end
                                                ...
                                                if year then origin = string.format("%s (%s)", origin, year) end
                                                    ...
                                                    mp.register_event("track-switched", notify_current_media)
